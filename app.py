    ...
    <script>
      function updateURL(lat, lon) {{
        try {{
            ...
            document.getElementById("msg").innerText = "回填失敗(跨域)，請手動複製";
        }}
      }}
      ...
      AMapLoader.load({{key:"{AMAP_JS_KEY}", version:"2.0"}}).then((AMap)=>{{
        ...
        ms.forEach(m => {{
            new AMap.Marker({{ position:[m.lon, m.lat], title:m.name }}).setMap(map);
        }});
