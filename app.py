...
with tabs[0]:
    ...
    if run_btn:
        ...
        # -----------------------------
        # Step C: Deep-dive prompt (content first) - 加入產業格局版
        # -----------------------------
...
- 請掃視新聞池，若有明確提及上游供應商或下游客戶，請以「公司名 (新聞關鍵字)」格式列出。
- 注意：若新聞池完全未提及任何供應鏈公司名，請直接輸出「本次新聞池未提供供應鏈資訊」，嚴禁自行發揮。
            """.strip()
        )
        ...
        你是一位買方避險基金（Buy-side Hedge Fund）的資深產業與風險研究員。你的 KPI 是「結合由上而下 (Top-Down) 的產業視角與由下而上 (Bottom-Up) 的數據，找出投資論點的漏洞與下行風險」。
        請根據下列資料，產出結構化、邏輯嚴密且具備批判性的分析報告。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 產業分析邊界：允許運用你對該產業的總經與商業模式認知，但【嚴禁捏造具體的市占率數字或財報未公布的數據】。
        2. 嚴禁編造絕對目標價：情境分析只能使用「跌破 MA20」、「乖離收斂」、「突破近期高點」等相對指標，【絕對禁止】憑空編造具體股價（如 2000、2200 等）。
        3. 來源精準對應：乖離率的來源必須寫「價格快照/MA20」，不可張冠李戴寫成大盤指數。
        4. 反證的正確定義：反證必須是「如果這件事發生，我的核心論點就會崩潰的證據」，嚴禁把支持論點的利多當成反證。
        5. 誠實面對缺漏：遇到「無資料」，直接寫「數據缺失無法評估」，絕對不可自行腦補數字或情境。
        ════════════════════════════════════════
        ...
        【輸出框架（請直接輸出標題與內容，不需要重複上述規則）】

        ### 1) Investment Thesis｜投資論述（Why now?）
        - 核心一句話：【必須以「偏多 / 偏空 / 短線觀望」開頭】，並用一句話說明最核心理由。
        - Variant Perception：市場現在瘋狂關注什麼？但根據數據與產業特性，市場可能忽略了什麼隱藏風險或未發酵的機會？
        - 關鍵變數：挑選 1 個決定此論點成敗的最重要變數（如：終端需求復甦、MA20 支撐）。

        ### 2) Industry Context & Moat｜產業格局與競爭力
        - 產業順風 (Tailwinds)：基於新聞池與【{industry}】當前的宏觀趨勢，指出 1 個正在推升該板塊的核心利多（如：AI 資本支出擴張、降息預期）。
        - 產業逆風 (Headwinds)：指出該產業目前面臨的 1 個最大系統性阻力或地緣/總經風險。
        - 護城河 / 主題潛力：該公司在產業中的核心競爭優勢為何？（若為 ETF 則分析該主題的長期不可替代性）。

        ### 3) Evidence & Counter-evidence｜證據與反證
        **【支持論點的證據鏈（3點）】**
        - 證據 1：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 2：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 3：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]

        **【推翻論點的最強反證（2點）】**
        - 反證 1：[描述一個與你論點相反的數據或新聞] ｜ 若此成立，論點將修正為：[說明]
        - 反證 2：[描述一個與你論點相反的數據或新聞] ｜ 若此成立，論點將修正為：[說明]

        ### 4) Valuation & Risk/Reward｜估值與技術面評估
        - 估值快評：根據 P/E、P/B 或 PEG，簡評目前估值水位（若無資料直接寫無法評估）。
        - 技術面解讀：現價相對 MA20 乖離為 {gap_pct:+.2f}%，請明確判斷這是屬於「過熱」、「中性」還是「超賣區間」。
        - 風險報酬框架：目前做多的最大上行催化劑是什麼？最大的下行風險觸發點是什麼？

        ### 5) Scenario Analysis｜情境推演（禁止編造具體股價）
        - 🟢 Bull (樂觀)：觸發條件為 [使用相對指標，例如突破前高或 MA20 乖離擴大至 X%] → 產業/估值預期影響為 [說明]。
        - 🟡 Base (基準)：目前數據最支持的路徑為 [說明]，佐證數據為 [引用輸入數字]。
        - 🔴 Bear (悲觀)：觸發條件為 [使用相對指標，例如跌破 MA20 且產業逆風加劇] → 風險將如何擴散 [說明]。

        ### 6) Catalysts & Monitoring Checklist｜催化劑與監測清單
        （列出 3 個未來需追蹤的具體事件，【必須與上述產業動態或估值數據具體相關】，格式如下）
        1. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若符合預期則代表 [利多/利空]。
        2. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若符合預期則代表 [利多/利空]。
        3. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若符合預期則代表 [利多/利空]。

        {supply_chain_section}

        ### 7) Action Plan｜行動方案
        - 風險控管思路：具體的操作建議（例如：乖離過高應等待回測 MA20，或跌破某數據應停損）。
        - 適合的投資者：[短線波段 / 中長線價值 / 收益型]，原因為 [說明]。
        - 關鍵數據驗證：本次報告最缺乏的數據是 [填寫]，取得後可進一步確認 [某個論點]。
        """)
