...
"""
        ...
        # 修正：將規則化為「輸出指令」，防止 AI 直接把規則當成內文印出來
...
- 注意：若新聞池完全未提及任何供應鏈公司名，請直接輸出「本次新聞池未提供供應鏈資訊」，嚴禁自行發揮。
        ...
        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 嚴禁自相矛盾：Thesis 若寫「過熱風險」，Evidence 就不能把「大漲」當作純利多，必須解讀為「推升風險」。
        2. 反證的正確定義：反證必須是「如果這件事發生，我的核心論點就會崩潰的證據」，嚴禁把支持論點的利多當成反證。
        3. 嚴禁照抄格式提示：不要把「例：站穩 MA20」這種說明文字印出來，必須根據當前標的數據自行填入真實條件。
        ...
        - 核心一句話：必須明確表達方向（偏多/偏空/觀望），並用一句話說明最核心理由。
        - Variant Perception：市場現在瘋狂關注什麼？但根據數據，市場可能忽略了什麼風險或機會？
        - 關鍵變數：挑選 1 個決定此論點成敗的最重要變數。

        ### 2) Evidence & Counter-evidence｜證據與反證
        **【支持論點的證據鏈（3點）】**
        （請用「輸入資料」或「新聞池」佐證你的 Thesis，格式如下）
        - 證據 1：[描述] ｜ 來源：[數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 2：[描述] ｜ 來源：[數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 3：[描述] ｜ 來源：[數據/新聞標題] ｜ 支撐強度：[高/中/低]
        ...
        **【推翻論點的最強反證（2點）】**
        ...
        ### 4) Scenario Analysis｜情境分析（必須設定具體觸發條件）
        - 🟢 Bull (樂觀)：觸發條件為 [請設定一個具體技術面或基本面門檻，例如股價突破某價位或營收轉正] → 預期影響為 [說明]。
        - 🟡 Base (基準)：目前數據最支持的路徑為 [說明]，佐證數據為 [引用輸入數字]。
        - 🔴 Bear (悲觀)：觸發條件為 [請設定具體跌破門檻，例如跌破 MA20 且乖離轉負] → 風險將如何擴散 [說明]。

        ### 5) Catalysts & Monitoring Checklist｜催化劑與監測清單
        （列出 3 個未來需追蹤的具體事件或數據，格式如下）
