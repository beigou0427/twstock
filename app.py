...
with tabs[0]:
    ...
    if run_btn:
        ...
        # -----------------------------
        # Step C: Deep-dive prompt (content first) - 機構級終極版
        # -----------------------------
...
- 成分股集中度：若資料不足，請明確輸出「需進一步取得成分股權重資料」；不得以常識推斷持股。
- 產業曝險：僅能根據「新聞池標題」推導方向；每個產業結論後需括號標註對應新聞標題關鍵字。
- Beta/波動特性：引用上方輸入的 Beta 值說明其相對於大盤的波動特性。
            """.strip()
            if is_etf else
            """
### 🔗 Supply Chain Dynamics｜產業鏈供需
- 僅允許列出「新聞池標題中明確出現」且「非本標的」的公司名稱，格式：公司名（新聞標題關鍵字）。
- 禁止：把標的自己列為自己的供應鏈、禁止憑空補充外部常識供應鏈。
- 若新聞池未出現其他公司名：請只輸出一句「本次新聞池未提供供應鏈資訊」並結束本段。
            """.strip()
        )
        ...
        你是賣方（Sell-side）頂級投行的資深股票研究分析師，負責撰寫給機構 PM 使用的深度報告。
        報告必須「內容充實、具備產業相對視角、每段都有實質數據綁定」，嚴禁空洞重複的廢話。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 數據強制綁定：每一個段落、每一個論點，都必須引用上方「輸入資料」中的具體數字（如 P/E、Beta、乖離率 {gap_pct:+.2f}%、目標價、殖利率等），不得只寫純敘事。
        2. 同業與產業規範：
           - 同業定位：允許使用輸入的 P/E、Beta、P/B 等數據，對標的在【{industry}】產業中做「定性比較」（如 Beta 偏低代表防禦性較強），但禁止編造同業的具體市佔率或數據。
           - 產業動向：順風/逆風必須引用新聞標題，並推導出「對產業的影響 → 對本標的營收/利潤的影響」。
        3. 語氣專業化：禁止使用「泡沫、暴漲、血洗、韭菜」等口語，改用「估值擴張、投資循環、風險偏好、均值回歸」等機構用語。
        4. 禁止臆造門檻：情境分析（Scenario）禁止寫出絕對股價點位；催化劑（Catalysts）指標只能從輸入資料的欄位中選取，沒有的資料請寫「待補充」。
        5. 立場絕對一致：封面摘要的評級（Buy/Neutral/Sell）必須貫穿全篇，反證必須能明確推翻評級，Action Plan 必須與評級邏輯完全吻合。
        ════════════════════════════════════════
        ...
        【輸入資料（寫作時必須不斷引用以下數字，沒有就寫『無資料』）】
        {price_text}
        {valuation_text}
        {dividend_ai_text}

        【全球新聞池】（{len(final_news)} 篇）：
        {news_summary}

        ════════════════════════════════════════
        【輸出框架（請直接輸出標題與內容，不需要重複上述規則文字）】
        ════════════════════════════════════════

        ### 🏦 Executive Summary｜封面摘要
        - 評級：【必須從 Buy / Neutral / Sell 擇一，後接確信度 High / Medium / Low】
        - 核心觀點：  
          為何此評級：[請用 1 個估值或技術數字 + 1 句產業邏輯來說明]。  
          最大風險：[請用 1 個數字或新聞 + 觸發條件來說明會讓你改變評級的因素]。
        - 三大投資亮點：（請用列點方式，每點必須綁定一個輸入數字或新聞）
          • [引用目標價/P/E/PEG等數字] 顯示其 [上行空間/估值吸引力/成長預期]。
          • [引用Beta/P/B/殖利率等數字] 確認其在產業中具備 [防禦性/成長性/資金避風港] 價值。
          • [引用乖離率/短期報酬等數字] 提供目前技術面 [進場/出場/觀望] 的具體訊號。

        ### 1) Industry Context & Moat｜產業格局與同業定位
        - 產業順風 (Tailwinds)：[引用 1 則新聞標題]，該趨勢對【{industry}】的影響為 [說明]，進而可望挹注本標的之 [營收/利潤/市佔率]。
        - 產業逆風 (Headwinds)：[引用 1 則新聞標題]，帶來的系統性阻力為 [說明]，對本標的的潛在挑戰為 [具體風險]。
        - 同業相對定位：基於本標的 Beta 值、P/E 及 P/B 水位，相對於【{industry}】同業，本標的屬於 [防禦型 / 積極成長型 / 價值修復型] 配置（註：因缺乏完整 peer group，此為基於現有數據之定性推估）。
        - 護城河：[結合上述數據或新聞]，說明其最難被同業複製的競爭優勢。

        ### 2) Investment Thesis｜投資論述（Why now?）
        - 核心一句話：[與封面評級一致的方向]，因為 [引用輸入的 1 個具體數字] 顯示目前的 [估值/技術/股利] 處於 [具體狀態]。
        - Variant Perception：市場共識可能認為 [引用某新聞或大眾觀點]；但本報告認為市場可能誤判了 [新視角]，關鍵理由是 [引用不同於市場觀點的數字或另一則新聞]。
        - 關鍵 Driver：決定此評級成敗的最核心變數是 [1 個可監控的真實指標]。

        ### 3) Evidence & Counter-evidence｜證據鏈與最強反證
        **【支持評級的證據（3點，每點務必引用不同的輸入數字或新聞）】**
        - 證據 1：[描述論點] ｜ 來源：[具體欄位名稱/新聞標題] ｜ 支撐強度：高/中/低
        - 證據 2：[描述論點] ｜ 來源：[具體欄位名稱/新聞標題] ｜ 支撐強度：高/中/低  
        - 證據 3：[描述論點] ｜ 來源：[具體欄位名稱/新聞標題] ｜ 支撐強度：高/中/低

        **【若此成立則評級需反轉的反證（2點，必須具體可驗證）】**
        - 反證 1：若發生 [具體數據惡化或產業事件]，評級將由 [目前評級] 轉為 [新評級] ｜ 來源指標：[具體欄位]
        - 反證 2：若發生 [具體數據惡化或產業事件]，評級將由 [目前評級] 轉為 [新評級] ｜ 來源指標：[具體欄位]

        ### 4) Valuation & Technical｜估值與技術面評估
        - 估值水位：目前 P/E、P/B 為 [填入數字]，此倍數顯示 [具體解讀]（註：因缺乏 EPS 成長率及毛利率等細部財報數據，此為靜態倍數快評）。
        - 技術面判斷：目前乖離率為 {gap_pct:+.2f}%，結合 MA20 趨勢，判定市場情緒屬於 [過熱/中性/超賣]，短期 MA20 視為 [支撐/壓力] 線。
        - 風險報酬比：最大的上行催化劑需見到 [某數字改善]；最大的下行觸發點為 [某數字惡化]。

        ### 5) Scenario Analysis｜三情境推演（禁止編造絕對股價點位）
        - 🟢 Buy Case：若乖離率由目前的 {gap_pct:+.2f}% 收斂/擴大 [X] 個百分點，且 [另一輸入數字改善] → 預期產業與估值影響為 [定性說明]。
        - 🟡 Base Case：目前數據最支持的路徑為維持現狀，佐證數字為 [引用 1 個輸入數字]。
        - 🔴 Sell Case：若失守 MA20 且乖離率進一步惡化 → 將引發風險擴散，預期影響為 [定性說明]。

        ### 6) Catalysts & Risk Matrix｜催化劑與風險矩陣
        **待觀察催化劑（3項，指標必須從輸入數據中選取，若無請寫「待補充財報數據」）**
        1. [事件名稱]｜監測指標：[輸入欄位名]｜判讀：若大於/小於現海水位，則視為 [利多/利空]
        2. [事件名稱]｜監測指標：[輸入欄位名]｜判讀：若大於/小於現海水位，則視為 [利多/利空]
        3. [事件名稱]｜監測指標：[輸入欄位名]｜判讀：若大於/小於現海水位，則視為 [利多/利空]

        **風險矩陣（發生率 × 影響度）**
        - 高發生率 × 高影響：[引用具體數字惡化的事件]
        - 中發生率 × 高影響：[引用新聞中的黑天鵝事件]
        - 高發生率 × 低影響：[短期的技術面或雜訊干擾]

        {supply_chain_section}

        ### 7) Action Plan｜行動方案
        - 建議策略：維持 [評級] 判斷，具體操作建議為 [結合 MA20、乖離率或現有估值的操作方式]。
        - 適合投資人：適合 [短線波段 / 中長線價值 / 收益防禦] 型投資人，主因為其 [引用 Beta 或殖利率] 之特性。
        - 下次更新觸發：本次報告最缺乏的關鍵數據為 [填寫 1 個財報指標，如 EPS YoY]，取得該數據後將重新檢視 [特定論述]。
        """)
