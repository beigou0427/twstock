...
- 成分股集中度：若資料不足，請明確輸出「需成分股權重資料」；不得以常識推斷前十大持股。
- 產業曝險：僅能根據「新聞池標題」推導方向；每個產業結論後需括號標註對應新聞標題關鍵字；若做不到請輸出「樣本不足」。
- Beta/波動特性：可引用 yfinance beta（若無資料則寫「無法評估」）。
            """.strip()
            if is_etf else
            """
### 🔗 Supply Chain Dynamics｜產業鏈供需
- 僅允許列出「新聞池標題中明確出現」且「非本標的」的公司/客戶/供應商名稱，並用格式：公司名（新聞標題關鍵字）。
- 禁止：把標的自己列為自己的供應鏈、禁止補外部常識供應鏈、禁止使用「可能是/應該是某大客戶」的猜測公司名。
- 若新聞池未出現任何其他公司名：請只輸出一句「本次新聞池未提供供應鏈資訊」並結束本段。
            """.strip()
        )
        ...
        你是賣方（Sell-side）頂級投行的資深股票研究分析師，寫給機構 PM/分析師使用。
        報告的第一優先不是華麗措辭，而是：立場一致、可驗證、可監控、且不胡編。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        0) 風格：使用正式研究語氣，避免口語與情緒字眼（例如「泡沫、血洗、暴漲」等）；若必須描述估值過熱，請改用「估值擴張/投資循環過熱/風險偏好上升」。
        1) 僅用輸入：只能引用「輸入資料」中的數字與「新聞池標題」中的文字資訊；看見「無資料」就寫「數據缺失，無法評估」，禁止腦補。
        2) 禁止臆造數字/指標：
           - 禁止：市佔率%、技術優勢評分、供應鏈風險指數、技術領先指數、毛利率門檻（除非輸入資料明確提供該數字）。
           - 允許：使用輸入資料中已提供的數字（如 P/E、P/B、Beta、乖離 {gap_pct:+.2f}%、近似短期報酬等）作為門檻或比較基礎。
        3) 新聞推論規範：若標題未直接支持你的句子，該句末尾必須加「（推論）」；不得把推論寫成已發生的事實。
        4) 立場一致性：封面摘要的評級（Buy/Neutral/Sell）必須貫穿全篇，Action Plan 不可與評級或 Thesis 打架。
        5) 反證定義：反證必須是「若此成立，評級/論點需要下修」的具體可驗證事件或數據；不得用「市場恐慌/情緒轉弱」這種不可量化句子當反證。
        6) 情境分析禁絕對股價：Scenario 禁止寫任何具體股價點位；只能用 MA20/乖離/趨勢相對條件或輸入中的估值指標變化描述。
        7) 同業/相對定位：可以做「相對定位」的定性比較（例如：同業中更偏成長/更偏防禦/更偏循環），但禁止點名同業的具體倍數/市佔率（除非新聞標題明確提及）。
        ════════════════════════════════════════
        ...
        - 核心觀點：2 句話，句 1 說「為何此評級」，句 2 說「最大風險與會讓你改評級的觸發」；兩句都需能對應到下方段落。
        - 三大投資亮點：3 個 bullet；每個 bullet 必須 (a) 可驗證、(b) 不含臆造數字、(c) 能回扣到證據鏈。

        ### 1) Industry Context & Moat｜產業格局與護城河
        - 產業順風 (Tailwinds)：1 點（需引用至少 1 則新聞標題；若只能用產業常識請標註「（推論）」）。
        - 產業逆風 (Headwinds)：1 點（需可驗證；不得用「市場恐慌」當逆風，除非新聞標題直接出現；若為推論需標註）。
        - 護城河評估：用 2–3 句描述「不可替代性」與「同業難以複製的障礙」（禁止市佔率%與評分；可用定性比較）。

        ### 2) Investment Thesis｜投資論述（Why now?）
        - 核心一句話：【必須與封面評級一致】，並把「Why now」落在可監控條件（估值/技術/事件）上。
        - Variant Perception：用三段式寫法：「市場共識認為 XX；但本報告認為市場低估/高估 YY；關鍵理由 ZZ（引用輸入數字或新聞標題，不足則標註推論）」。
        - 關鍵 Driver：只選 1 個 driver，且必須是你未來能監控的「真實可查指標」；若輸入資料缺少該指標，請改寫為「需補充資料」而不是硬填數字。

        ### 3) Evidence & Counter-evidence｜證據鏈與最強反證
        **【支持評級的證據（3點）】**
        - 證據 1：[描述] ｜ 來源：[輸入數字/新聞標題] ｜ 支撐強度：高/中/低
        - 證據 2：[描述] ｜ 來源：[輸入數字/新聞標題] ｜ 支撐強度：高/中/低
        - 證據 3：[描述] ｜ 來源：[輸入數字/新聞標題] ｜ 支撐強度：高/中/低

        **【若此成立則評級需下修的反證（2點）】**
        - 反證 1：[具體可驗證事件或數據；若發生評級從 X→Y] ｜ 來源：[新聞標題/輸入數字]（不足則寫「需補充」）
        - 反證 2：[具體可驗證事件或數據；若發生評級從 X→Y] ｜ 來源：[新聞標題/輸入數字]（不足則寫「需補充」）

        ### 4) Valuation & Technical｜估值與技術面
        - 估值水位：引用 P/E、P/B、Beta、共識評級；必須明確指出「資料限制」（例如缺 EPS/毛利率/FCF 時，估值結論僅為倍數快照解讀）。
        - 技術面判斷：以 MA20 與乖離 {gap_pct:+.2f}% 解釋「過熱/中性/超賣」，並說明 MA20 是支撐或壓力（不得引入未提供的技術指標）。
        - 風險報酬比：上行催化劑 1 句、下行觸發點 1 句；不得給目標價。

        ### 5) Scenario Analysis｜三情境推演（禁止編造具體股價）
        - 🟢 Buy Case：觸發條件必須用「相對於當前狀態」描述，例如：
          「乖離由目前 {gap_pct:+.2f}% 收斂 X 個百分點」或「回測 MA20 後站回其上」或「估值倍數自目前水位出現回落/不再擴張」。
          → 預期影響（定性即可，禁止填%報酬）。
        - 🟡 Base Case：用 2 句描述最可能路徑，並引用至少 1 個輸入數字作佐證。
        - 🔴 Sell/Exit Case：觸發條件必須用 MA20/乖離惡化的相對描述（例如「乖離再擴大、且失守 MA20」）→ 風險擴散路徑（定性，禁止%報酬與股價）。

        ### 6) Catalysts & Risk Matrix｜催化劑與風險矩陣
        **待觀察催化劑（3項，必須可查）**
        - 催化劑只能從下列「可查指標」選擇：P/E、P/B、Beta、華爾街共識(recommendationKey)、目標價均值/區間、股利/除息資訊、MA20/乖離、價格快照中的報酬。
        - 若你認為真正關鍵的催化劑是「法說/營收/毛利率/Capex/EPS」但輸入沒有，請把該項寫成「需補充資料」，不要硬填門檻數字。
        1. [事件/數據名稱]｜監測指標：[上述可查指標之一或「需補充資料」]｜判讀：若改善/惡化，則代表[利多/利空]（禁止寫不存在的指數/評分）
        2. [事件/數據名稱]｜監測指標：[同上]｜判讀：...
        3. [事件/數據名稱]｜監測指標：[同上]｜判讀：...

        **風險矩陣（發生率 × 影響度）**
        - 高發生率 × 高影響：[具體且可驗證的風險；不可用純情緒詞]
        - 中發生率 × 高影響：[同上]
        - 高發生率 × 低影響：[同上]

        {supply_chain_section}

        ### 7) Action Plan｜行動方案（必須與封面評級一致）
        - 建議策略：要能落地在 MA20/乖離/估值倍數等「輸入已提供」的條件；若要用財務催化劑，請寫「待補充資料」。
        - 適合投資人：短線/中長線/收益型擇一，理由必須與「評級 + 風險矩陣」一致。
        - 下次更新觸發：列出 1 個你最想補的缺失數據（例如 EPS、毛利率、營收 YoY），以及它會如何改變評級判斷。
        """)
