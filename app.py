...
with tabs[0]:
    ...
    if run_btn:
        ...
        prog.progress(65)
           ...
           # -----------------------------
        # Step C: Deep-dive prompt (content first) - 最終穩定版
        # -----------------------------
...
- 注意：若新聞池完全未提及任何供應鏈公司名，請直接輸出「本次新聞池未提供供應鏈資訊」，嚴禁自行發揮。
        ...
        你是一位買方避險基金（Buy-side Hedge Fund）的風險研究員。你的 KPI 是「找出投資論點的漏洞與下行風險」，而不是推銷股票。
        請根據下列資料，產出結構化、邏輯嚴密且具備批判性的分析報告。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 嚴禁編造絕對目標價：所有分析（特別是情境分析）只能使用「跌破 MA20」、「乖離收斂至 X%」、「突破近期高點」等相對指標，【絕對禁止】憑空編造具體股價（如 2000、2200 等）。
        2. 來源精準對應：乖離率的來源必須寫「價格快照/MA20」，不可張冠李戴寫成大盤指數。
        3. 反證的正確定義：反證必須是「如果這件事發生，我的核心論點就會崩潰的證據」，嚴禁把支持論點的利多當成反證。
        4. 誠實面對缺漏：遇到「無資料」，直接寫「數據缺失無法評估」，絕對不可自行腦補數字或情境。
        ════════════════════════════════════════
        ...
        - Variant Perception：市場現在瘋狂關注什麼？但根據數據，市場可能忽略了什麼風險或機會？
        - 關鍵變數：挑選 1 個決定此論點成敗的最重要變數（如：營收 YoY、MA20 支撐）。

        ### 2) Evidence & Counter-evidence｜證據與反證
        **【支持論點的證據鏈（3點）】**
        （請用「輸入資料」或「新聞池」佐證你的 Thesis，格式如下）
        - 證據 1：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 2：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]
        - 證據 3：[描述] ｜ 來源：[精準對應的輸入數據/新聞標題] ｜ 支撐強度：[高/中/低]

        **【推翻論點的最強反證（2點）】**
        （如果這兩點成立，你的 Thesis 就會錯。格式如下）
        ...
        ### 4) Scenario Analysis｜情境分析（禁止編造具體股價）
        - 🟢 Bull (樂觀)：觸發條件為 [使用相對指標，例如突破前高或 MA20 乖離擴大至 X%] → 預期影響為 [說明]。
        - 🟡 Base (基準)：目前數據最支持的路徑為 [說明]，佐證數據為 [引用輸入數字]。
        - 🔴 Bear (悲觀)：觸發條件為 [使用相對指標，例如跌破 MA20 且乖離轉負] → 風險將如何擴散 [說明]。

        ### 5) Catalysts & Monitoring Checklist｜催化劑與監測清單
        （列出 3 個未來需追蹤的具體事件，【必須與上述新聞池或估值數據具體相關】，格式如下）
        1. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若大於/小於某預期，則代表 [利多/利空]。
        2. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若大於/小於某預期，則代表 [利多/利空]。
        3. [事件/數據名稱]｜監測指標：[具體觀察什麼]｜判讀方式：若大於/小於某預期，則代表 [利多/利空]。
