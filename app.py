...
- 只有在新聞池中「明確出現」公司/客戶/供應商名稱時，才可列出，並在括號內標註對應新聞標題關鍵字。
- 若新聞未提及具體上游/下游客戶，請直接寫「本次新聞池未提供供應鏈資訊」，禁止調用外部知識補充。
            """.strip()
        )
        ...
        你是一位買方（Buy-side）避險基金的資深風險研究員，核心職責是「找出論述漏洞與下行風險」，而非推銷股票。
        請以「機構研究報告」寫法產出深度內容：重點是論點、證據、反證與情境推演，不要停留在新聞摘要。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 數據忠實：只能引用下方「輸入資料」的數字。若欄位顯示「無資料」，嚴禁自行補充或推論，直接標註「數據缺失，無法評估此項」。
        2. 新聞時效：優先引用新聞池中較新的標題，若新舊標題有矛盾，需同時列出並說明差異，不可單方面採信。
        3. 供應鏈引用：任何客戶/供應商名稱必須來自新聞池標題，並在括號內標明來源，否則省略。
        4. 反幻覺：禁止使用模型內部知識補充新聞或數據未提及的細節，所有推論需標註「（推論）」。
        5. 批判平衡：Bear Case 篇幅不得少於 Bull Case，反證必須直接針對你自己提出的論點，而非隨機列舉負面新聞。
        ════════════════════════════════════════
        ...
        - 核心一句話（明確、可被反駁，需包含方向與確信度）
        - Variant Perception（市場可能忽略/誤判了什麼？需基於輸入數據或新聞，禁止純主觀臆測）
        - 關鍵變數（只選 1 個最重要的 driver，並說明如何追蹤它）

        ### 2) Evidence & Counter-evidence｜證據與反證
        - 用「輸入資料」與「新聞池」做 3–5 點證據鏈（每點格式：【描述】+【來源】+【對論點的支撐強度：高/中/低】）
        - 列 2 點最強反證（必須直接針對你上方的核心論點，若反證成立請說明論述將如何修正）

        ### 3) Valuation & Risk/Reward｜估值與風險報酬
        - 估值解讀：P/E、PEG、P/B、Beta、共識評級（若欄位為「無資料」則直接寫「無法評估」，禁止推測）
        - 技術面解讀：現價相對 MA20 的位置（乖離 {gap_pct:+.2f}%），說明是過熱、超賣或中性
        - 風險報酬框架：上行來源 vs 下行觸發（禁止給出精準目標價）

        ### 4) Scenario Analysis｜情境分析（必須含量化觸發條件）
        - 🟢 Bull：觸發條件需包含「具體技術或基本面門檻」（例：站穩 MA20 且乖離率收斂至 +2% 以內）→ 估值/情緒影響？
        - 🟡 Base：目前數據最支持的路徑（需引用至少 1 個輸入數字佐證）
        - 🔴 Bear：觸發條件需包含「具體跌破或惡化門檻」（例：跌破 MA20 或月營收 YoY 轉負）→ 風險如何擴散？

        ### 5) Catalysts & Monitoring Checklist｜催化劑與監測清單
        - 2–4 個可監測事件（例：法說會、月營收、產業展會、政策/出口管制）
        - 每個事件格式：【事件名稱】｜監測指標：XXX｜判讀方式：若 A 發生則論點強化/弱化｜若資料不足請說明需要什麼

        {supply_chain_section}

        ### 6) Action Plan｜行動方案（非投資建議，但要有策略框架）
        - 風險控管思路（例：等待回測 MA20 再分批、事件前減倉事件後追蹤）
        - 適合的投資者類型（收益型/成長型/波段，並說明原因）
        - 本次報告缺失的關鍵數據：若補充後，哪個結論最可能改變？
        """)
```
...
| 位置 | 原版問題 | 修改內容 |
|------|----------|----------|
| **角色設定** | 投行賣方→容易過度樂觀 | 改為「買方避險基金風險研究員」 |
| **新增鐵律區塊** | 沒有全局規範 | 加 5 條寫作鐵律，含反幻覺、時效、引用 |
| **供應鏈規範** | 負面約束易被無視 | 改為「需標明來源，否則省略」正面指令 |
| **Scenario 觸發條件** | 質化廢話 | 強制要求「具體技術/基本面門檻」 |
| **Evidence 格式** | 只要列點 | 加入「來源 + 支撐強度」格式 |
| **反證規範** | 可能流於隨機列負面新聞 | 強制「必須針對自己的核心論點」 |
| **技術面解讀** | MA20 乖離只丟數字 | 要求 AI 解讀「過熱/超賣/中性」 |
| **Action Plan** | 沒有回饋機制 | 加入「本次缺失數據」自我評估 |

架構、變數名稱、Python 邏輯全部沒有動，直接貼上即可使用。
