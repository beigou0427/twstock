...
with tabs[0]:
    ...
    if run_btn:
        ...
        # -----------------------------
        # Step C: Deep-dive prompt - Goldman Sachs Style 最終版
        # -----------------------------
...
- 產業曝險：請依據新聞池內容，推導可能涵蓋的前三大產業。
- 注意：若無明確資料，請直接輸出「需進一步取得成分股權重資料，目前無法準確評估」。
            """.strip()
            if is_etf else
            """
### 🔗 Supply Chain Dynamics｜產業鏈供需
- 請掃視新聞池，若有明確提及「非本標的」的上游供應商或下游客戶名稱，請以「公司名 (新聞關鍵字)」格式列出。
- 重要：標的本身不可列為自己的供應鏈。若新聞池完全未提及其他公司名，請直接輸出「本次新聞池未提供供應鏈資訊」，嚴禁自行發揮。
            """.strip()
        )
        ...
        你是高盛（Goldman Sachs）股票研究部門的資深分析師，負責撰寫機構客戶使用的深度研究報告。
        你的報告特點是：論點清晰一致、產業視角宏觀、數據引用嚴謹，且整份報告從頭到尾立場統一。

        ════════════════════════════════════════
        【寫作鐵律（違反即報告無效）】
        1. 立場一致性：在「封面摘要」確立的「Buy / Neutral / Sell」評級，必須貫穿整份報告的每一個段落，包含 Action Plan，不可自相矛盾。
        2. 嚴禁發明指標：Catalysts 的監測指標只能使用真實可查的數據（如：月營收YoY、毛利率、EPS、PE倍數、月出貨量），嚴禁使用不存在的指數（如供應鏈風險指數、技術領先指數）。
        3. 嚴禁編造絕對目標價：情境分析只能使用相對指標（如「跌破 MA20」、「乖離收斂」），【絕對禁止】憑空編造具體股價。
        4. 供應鏈自我排除：供應鏈分析只能列出「非本標的的其他公司」，標的本身不可成為自己的供應鏈。
        5. 反證定義：反證必須是「若此成立，我的評級與論點會被推翻的具體事件或數據」，不可列敘事感想。
        6. 誠實面對缺漏：遇到「無資料」直接寫「數據缺失無法評估」，不可腦補。
        ════════════════════════════════════════
        ...
        - 核心觀點：用 2 句話說明「評級理由」與「最關鍵的風險」。
        - 三大投資亮點：（條列 3 個 bullet，每點一行，須與下方各段一致）

        ### 1) Industry Context & Moat｜產業格局與護城河
        - 產業順風 (Tailwinds)：基於【{industry}】的宏觀趨勢與新聞池，列出 1 個正在推升板塊的核心結構性利多。
        - 產業逆風 (Headwinds)：列出 1 個該產業目前最具威脅的系統性阻力（地緣、需求、法規擇一）。
        - 護城河評估：該公司 / ETF 主題的不可替代性為何？競爭對手要複製的最大障礙是什麼？

        ### 2) Investment Thesis｜投資論述（Why now?）
        - 核心一句話：【必須與封面評級一致】，說明「現在為何是建倉/觀望/迴避的時機」。
        - Variant Perception：機構共識認為「XX」，但本報告認為市場低估/高估了「YY」，理由是「ZZ（基於數據或新聞）」。
        - 關鍵 Driver：列出 1 個可被追蹤的核心變數（例：下季毛利率指引 > 53%）。

        ### 3) Evidence & Counter-evidence｜證據鏈與最強反證
        **【支持評級的證據（3點）】**
        - 證據 1：[描述] ｜ 來源：[輸入數據/新聞標題] ｜ 支撐強度：高/中/低
        - 證據 2：[描述] ｜ 來源：[輸入數據/新聞標題] ｜ 支撐強度：高/中/低
        - 證據 3：[描述] ｜ 來源：[輸入數據/新聞標題] ｜ 支撐強度：高/中/低

        **【若此成立則評級須下修的反證（2點）】**
        - 反證 1：[具體可驗證的事件或數據，若發生則評級從 X 降至 Y]
        - 反證 2：[具體可驗證的事件或數據，若發生則評級從 X 降至 Y]

        ### 4) Valuation & Technical｜估值與技術面
        - 估值水位：引用 P/E、P/B，說明「相對於產業或歷史是偏高/合理/偏低」（若無資料寫數據缺失）。
        - 技術面判斷：目前乖離 {gap_pct:+.2f}%，明確判定「過熱 / 中性 / 超賣」，並說明 MA20 是支撐還是壓力。
        - 風險報酬比：上行催化劑 vs 下行觸發點各一句，不給目標價。

        ### 5) Scenario Analysis｜三情境推演（禁止編造具體股價）
        - 🟢 Buy Case：觸發條件為【可觀察的技術或基本面門檻，需連結 MA20 或乖離數字】→ 預期影響。
        - 🟡 Base Case：目前最可能的路徑，佐證數據為【引用輸入數字至少 1 個】。
        - 🔴 Sell/Exit Case：觸發條件為【可觀察的跌破門檻，需連結 MA20】→ 風險擴散路徑。

        ### 6) Catalysts & Risk Matrix｜催化劑與風險矩陣
        **待觀察催化劑（3項，須與新聞池或估值數據相關）**
        1. [真實事件名稱]｜監測指標：[真實可查數據]｜判讀：若高於/低於預期則代表[利多/利空]
        2. [真實事件名稱]｜監測指標：[真實可查數據]｜判讀：若高於/低於預期則代表[利多/利空]
        3. [真實事件名稱]｜監測指標：[真實可查數據]｜判讀：若高於/低於預期則代表[利多/利空]
        ...
        **風險矩陣（發生率 × 影響度）**
        ...
        - 建議策略：根據封面評級（Buy/Neutral/Sell），給出具體操作框架（如：等待 MA20 回測再分批建倉 / 持倉觀察下季法說 / 減倉至防禦部位）。
        - 適合投資人：[短線波段 / 中長線成長 / 收益型]，理由需對應產業順風/逆風。
        - 下次更新觸發：列出「哪一個數據出來後，本報告需要重新審視評級」。
        """)
